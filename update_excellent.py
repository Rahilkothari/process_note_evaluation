...
load_dotenv()
from models.database import get_db, ProcessNote, ProcessSection, ValidationRun, ValidationFinding

db = next(get_db())

def update_excellent():
    note = db.query(ProcessNote).filter(ProcessNote.process_name.like('%EXCELLENT%')).first()
    if not note:
        print("Excellent note not found!")
        return

    # Delete existing sections for this note
    ...
    add_text("1.11", "This process applies to all full-time and part-time internal employees joining the company globally. It excludes contractors, freelancers, and temporary staff, who fall under the Vendor Onboarding Process (POL-HR-005).")
    ...
    # 1.9 Introduction
    add_text("1.14", "A seamless onboarding experience is critical for early employee engagement and rapid time-to-productivity. This process governs the cross-departmental coordination required to successfully integrate a new hire into the organization.")

    # 1.10 Objective
    add_text("1.15", "To establish a standardized, compliant, and efficient onboarding process for all new hires, ensuring they receive necessary IT assets, compliance training, and HR orientation within their first 48 hours, thereby minimizing time-to-productivity.")
